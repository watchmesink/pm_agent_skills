...
- Builds a short current frame, then keeps asking fresh, non-repeated clarifying questions until the user explicitly asks to craft the PRD.
- Focuses questions on problem space, jobs to be done, success criteria, high-level solution, and scope boundaries.
- Does not treat a clear frame as permission to draft. `Stop asking questions` alone is not enough unless the user also asks to start the PRD.
...
Ask Codex to use `$write-prd` when you want to frame, draft, or revise a PRD. Answer the discovery questions for as many rounds as useful, then explicitly say `draft the PRD`, `craft the PRD`, or `write the PRD` when you want the draft to start.
